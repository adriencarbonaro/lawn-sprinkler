...
#define DISCO_PREFIX "homeassistant"
...
static void publish_zone(uint8_t i)
{
    ...
    snprintf(topic,
             sizeof(topic),
             DISCO_PREFIX "/switch/" DEV_ID "/zone_%u/config",
             i);
    ...
    snprintf(payload,
             ...
             "{"
             "\"name\":\"Zone %u\","
             "\"unique_id\":\"" DEV_ID "_zone_%u\","
             "\"object_id\":\"" DEV_ID "_zone_%u\","
             "\"command_topic\":\"" T_ZONE_PREFIX "%u/set\","
             "\"state_topic\":\"" T_ZONE_PREFIX "%u/state\","
             "\"payload_on\":\"ON\","
             "\"payload_off\":\"OFF\","
             "\"state_on\":\"ON\","
             "\"state_off\":\"OFF\","
...
{
    const char* topic =
        DISCO_PREFIX "/select/" DEV_ID "/mode/config";

    const char* payload =
        "{"
        "\"name\":\"Mode\","
        "\"unique_id\":\"" DEV_ID "_mode\","
        "\"object_id\":\"" DEV_ID "_mode\","
